...
public class RegistrationManagerTest {
	...
	public void testGetCourseCatalog() {
		...
		manager.setCourseCatalog(c);
		assertEquals(c, manager.getCourseCatalog());
	}
	...
	public void testGetStudentDirectory() {
		...
		manager.setStudentDirectory(s);
		assertEquals(s, manager.getStudentDirectory());
	}
	...
	public void testLogin() {
		...
		manager.logout();
		assertTrue(manager.login("dnolan", "pw"));
		...
		try {
			manager.login("Nope", "StillNope");
			fail();
		} catch (IllegalArgumentException iae) {
			assertEquals("User doesn't exist.", iae.getMessage());
		}
	...
	public void testGetCurrentUser() {
		manager.logout();
		StudentDirectory s = new StudentDirectory();
		s.addStudent("John", "Doe", "jdoe", "jdoe@ncsu.edu", "password", "password", 12);
		manager.setStudentDirectory(s);
		manager.login("jdoe", "password");
	...
	/**
	 * Tests adding faculty to a course
	 */
	@Test
	public void testAddFacultyToCourse() {
		RegistrationManager regManager = RegistrationManager.getInstance();
		regManager.getFacultyDirectory().loadFacultyFromFile("test-files/faculty_records.txt");
		regManager.getCourseCatalog().loadCoursesFromFile("test-files/actual_course_records.txt");
		try {
			regManager.addFacultyToCourse(regManager.getCourseCatalog().getCourseFromCatalog("CSC116", "001"),
					regManager.getFacultyDirectory().getFacultyById("sesmith5"));
			fail();
		} catch (IllegalArgumentException e) {
			assertNull(regManager.getCurrentUser());
		}
		try {
			regManager.login("registrar", "Regi5tr@r");
			Faculty faculty = regManager.getFacultyDirectory().getFacultyById("awitt");
			Course course = regManager.getCourseCatalog().getCourseFromCatalog("CSC116", "003");
			course.setInstructorId(null);
			assertTrue(regManager.addFacultyToCourse(course, faculty));
		} catch (IllegalArgumentException e) {
			// skip this  line
		}
	}
	...
	/**
	 * tests removing faculty members from a course
	 */
	@Test
	public void testRemoveFacultyFromCourse() {
		RegistrationManager registration = RegistrationManager.getInstance();
		registration.getFacultyDirectory().loadFacultyFromFile("test-files/faculty_records.txt");
		registration.getCourseCatalog().loadCoursesFromFile("test-files/actual_course_records.txt");

		try {
			registration.addFacultyToCourse(registration.getCourseCatalog().getCourseFromCatalog("CSC116", "001"),
					registration.getFacultyDirectory().getFacultyById("sesmith5"));
			fail();
		} catch (IllegalArgumentException e) {
			assertNull(registration.getCurrentUser());
		}

		try {
			registration.removeFacultyFromCourse(registration.getCourseCatalog().getCourseFromCatalog("CSC116", "001"),
					registration.getFacultyDirectory().getFacultyById("sesmith5"));
			fail();
		} catch (IllegalArgumentException e) {
			assertNull(registration.getCurrentUser());
		...
		registration.login("registrar", "Regi5tr@r");
		Faculty f = registration.getFacultyDirectory().getFacultyById("awitt");
		Course c = registration.getCourseCatalog().getCourseFromCatalog("CSC116", "003");
		c.setInstructorId(null);

		assertTrue(registration.addFacultyToCourse(c, f));

		assertTrue(registration.removeFacultyFromCourse(c, f));
	}
}