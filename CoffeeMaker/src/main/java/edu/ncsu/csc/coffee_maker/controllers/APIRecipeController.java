...
public class APIRecipeController extends APIController {
    ...
    /**
     * REST API method to allow editing a Recipe in the CoffeMaker's inventory,
     * by making a PUT request to the API endpoint and indicating the recipe to
     * delete (as a path variable) and the new recipe state (part of the request
     * body).
     *
     * @param name
     *            name of the recipe to edit
     * @param r
     *            the new recipe state
     * @return Success if the recipe could be edited, an error if the recipe
     *         does not exist
     */
    @PutMapping ( BASE_PATH + "/recipes/{id}" )
    public ResponseEntity putRecipe ( @PathVariable ( "id" ) final String name, @RequestBody final Recipe r ) {
        final Recipe recipe = Recipe.getByName( r.getName() );
        if ( null == recipe ) {
            return new ResponseEntity( errorResponse( "No recipe found for name " + r.getName() ),
                    HttpStatus.NOT_FOUND );
        }
        recipe.updateRecipe( r );
        recipe.save();
        return new ResponseEntity( successResponse( recipe.getName() + " was updated successfully" ), HttpStatus.OK );
