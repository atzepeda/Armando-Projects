...
public class AppointmentRequestStepDefs extends CucumberTest {
    ...
    @Given ( "There is a sample Optometrist and sample Ophthalmologist in the database" )
    public void startingOphthalmologyUsers () {
        attemptLogout();

        final User optometrist = new User( "optometrist",
                "$2a$10$EblZqNptyYvcLm/VwDCVAuBjzZOI7khzdyGPBr08PpIi0na624b8.", Role.ROLE_HCP,
                Specialization.OPTOMETRIST, 1 );
        optometrist.save();

        final User ophthalmologist = new User( "ophthalmologist",
                "$2a$10$EblZqNptyYvcLm/VwDCVAuBjzZOI7khzdyGPBr08PpIi0na624b8.", Role.ROLE_HCP,
                Specialization.OPHTHALMOLOGIST, 1 );
    ...
    public void fillFields () {
        ...
        final Select type = new Select( driver.findElement( By.id( "type" ) ) );
    ...
    public void requestOphthalmologyAppointment () {
        waitForAngular();

        final Select type = new Select( driver.findElement( By.id( "type" ) ) );
        type.selectByVisibleText( "OPHTHALMOLOGY_APPOINTMENT" );

        final Select hcp = new Select( driver.findElement( By.id( "hcp" ) ) );
        hcp.selectByVisibleText( "optometrist" );
        ...
        final WebElement comments = driver.findElement( By.id( "comments" ) );
        comments.clear();
        comments.sendKeys( "Test ophthalmology appointment please ignore" );
        driver.findElement( By.name( "submit" ) ).click();
    }
    ...
    public void tryIncorrectHCP () {
        waitForAngular();

        final Select type = new Select( driver.findElement( By.id( "type" ) ) );
        type.selectByVisibleText( "OPHTHALMOLOGY_SURGERY" );

        final Select hcp = new Select( driver.findElement( By.id( "hcp" ) ) );
        final List<WebElement> validHCPs = hcp.getAllSelectedOptions();
        assertTrue( validHCPs.size() == 1 );
    ...
    public void createOphthalmologyAppointmentRequest () {
        attemptLogout();
        ...
        ar.setComments( "Test request" );
        ar.setPatient( User.getByNameAndRole( "patient", Role.ROLE_PATIENT ) );
        ar.setHcp( User.getByNameAndRole( "ophthalmologist", Role.ROLE_HCP ) );
        final Calendar time = Calendar.getInstance();
        time.setTimeInMillis( Calendar.getInstance().getTimeInMillis() + 1000 * 60 * 60 * 24 * 14 );
        ar.setDate( time );
        ar.setStatus( Status.PENDING );
        ar.setType( AppointmentType.OPHTHALMOLOGY_APPOINTMENT );
        ar.save();
    }
    ...
    public void loginOphthalmologist () {
        driver.get( baseUrl );
        final WebElement username = driver.findElement( By.name( "username" ) );
        username.clear();
