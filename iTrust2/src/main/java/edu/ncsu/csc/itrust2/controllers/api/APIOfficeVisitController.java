...
public class APIOfficeVisitController extends APIController {
    ...
    public List<OfficeVisit> getOfficeVisits () {
        ...
        final List<OfficeVisit> allVisits = OfficeVisit.getOfficeVisits();
        final List<OfficeVisit> generalVisits = new ArrayList<OfficeVisit>();
        final List<OfficeVisit> optoVisits = new ArrayList<OfficeVisit>();

        for ( final OfficeVisit visit : allVisits ) {
            if ( visit.getType() == null || visit.getType() == AppointmentType.GENERAL_CHECKUP ) {
                generalVisits.add( visit );
                optoVisits.add( visit );
            }
            if ( visit.getType() == AppointmentType.OPHTHALMOLOGY_APPOINTMENT ) {
                optoVisits.add( visit );
            }
        }

        if ( self != null && self.getSpecialization() == null ) {
            return generalVisits;
        }
        if ( self != null && self.getSpecialization() == Specialization.OPTOMETRIST ) {
            return optoVisits;
        }
        if ( self != null && self.getSpecialization() == Specialization.OPHTHALMOLOGIST ) {
            return allVisits;
        }
        return null;
    }
    ...
    public ResponseEntity updateOfficeVisit ( @PathVariable final Long id, @RequestBody final OfficeVisitForm form ) {
        try {
            ...
            if ( dbVisit.getBasicHealthMetrics() != null && visit.getBasicHealthMetrics() != null
                    && !dbVisit.getBasicHealthMetrics().equals( visit.getBasicHealthMetrics() ) ) {
