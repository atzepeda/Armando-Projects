...
public class OfficeVisit extends DomainObject<OfficeVisit> {
    ...
    public OfficeVisit ( final OfficeVisitForm ovf ) throws ParseException, NumberFormatException {
        ...
        if ( at == AppointmentType.OPHTHALMOLOGY_APPOINTMENT ) {
            if ( getHcp().getSpecialization() != Specialization.OPHTHALMOLOGIST
                    && getHcp().getSpecialization() != Specialization.OPTOMETRIST ) {
                throw new IllegalArgumentException( "Cannot add this type of AppointmentRequest to this HCP." );
            }
        ...
        Surgeries surg = null;
        try {
            surg = Surgeries.valueOf( ovf.getSurgery() );
        }
        catch ( final NullPointerException npe ) {
            surg = Surgeries.NONE; /*
                                    * If for some reason we don't have a
                                    * surgery, default to "None"
                                    */
        }
        if ( surg != Surgeries.NONE && getHcp().getSpecialization() != Specialization.OPHTHALMOLOGIST ) {
            throw new IllegalArgumentException( "The HCP is not valid fot this office visit." );
        }
        setSurgery( surg );
        ...
        if ( Hospital.getByName( ovf.getHospital() ) == null ) {
            throw new IllegalArgumentException( "The name of hospital is required." );
        }
        ...
        final BasicHealthMetrics bhm = new BasicHealthMetrics( ovf );
        if ( bhm == null ) {
            throw new IllegalArgumentException( "BasicHealthMetrics cannot be null." );
        }
        setBasicHealthMetrics( bhm );

        final OpticalHealthMetrics ohm = new OpticalHealthMetrics( ovf );
        if ( ohm == null ) {
            throw new IllegalArgumentException( "OpticalHealthMetrics cannot be null." );
        }
        setOpticalHealthMetrics( ohm );
        ...
        if ( age < 3 && type == AppointmentType.GENERAL_CHECKUP ) {
            validateUnder3();
        }
        else if ( age < 12 && type == AppointmentType.GENERAL_CHECKUP ) {
            validateUnder12();
        }
        else if ( type == AppointmentType.GENERAL_CHECKUP ) {
            validate12AndOver();
        }

        if ( type == AppointmentType.OPHTHALMOLOGY_APPOINTMENT || type == AppointmentType.OPHTHALMOLOGY_SURGERY ) {
            validateOphthalmology();
        }
        validateDiagnoses();
    ...
    private void validateOphthalmology () {
        final OpticalHealthMetrics ohm = getOpticalHealthMetrics();
        if ( ohm.getVisualAcuityLeft() == null || ohm.getVisualAcuityRight() == null || ohm.getSphereLeft() == null
                || ohm.getSphereRight() == null ) {
            throw new IllegalArgumentException( "Not all necessary fields for optical health metrics were submitted." );
        }
        if ( ( ohm.getCylinderLeft() != null || ohm.getCylinderRight() != null )
                && ( ohm.getAxisLeft() == null || ohm.getAxisRight() == null ) ) {
    ...
    /**
     * Get the surgery for this office visit
     *
     ...
     */
    public void setSurgery ( final Surgeries surgery ) {
    ...
    /**
     * Gets the optical health metrics for this office visit
     *
     * @return the optical health metrics
     */
    public OpticalHealthMetrics getOpticalHealthMetrics () {
        return opticalHealthMetrics;
    }

    /**
     * Sets the optical health metrics for this office visit
     *
     * @param ohm
     *            the optical health metrics to set
     */
    public void setOpticalHealthMetrics ( final OpticalHealthMetrics ohm ) {
        this.opticalHealthMetrics = ohm;
    }
    ...
    /**
     * The optical health metric data associated with this office visit.
     */
    @OneToOne
    @JoinColumn ( name = "opticalhealthmetrics_id" )
    private OpticalHealthMetrics         opticalHealthMetrics;
    ...
    @Override
    public void save () {
        BasicHealthMetrics oldBhm = null;
        if ( basicHealthMetrics != null ) {
            oldBhm = BasicHealthMetrics.getById( basicHealthMetrics.getId() );
            this.basicHealthMetrics.save();
        }

        OpticalHealthMetrics oldOhm = null;
        if ( opticalHealthMetrics != null ) {
            oldOhm = OpticalHealthMetrics.getById( opticalHealthMetrics.getId() );
            this.opticalHealthMetrics.save();
        }
        //// SAVE PRESCRIPTIONS ////
